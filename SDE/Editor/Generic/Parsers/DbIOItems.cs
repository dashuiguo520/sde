...
namespace SDE.Editor.Generic.Parsers {
	public sealed class DbIOItems {
		...
		public static void Loader<TKey>(DbDebugItem<TKey> debug, AbstractDb<TKey> db) {
            ...
            // 在 weaponFlags 和 ammoFlags 字典定义附近添加 cardFlags
            var cardFlags = new Dictionary<string, long>();
            cardFlags["normal"] = 0x0;     // 默认卡片类型
            cardFlags["enchant"] = 0x1;    // 附魔卡片类型
			...
			if (debug.FileType == FileType.Txt) {
				...
				foreach (var item in ele.Output["copy_paste"] ?? ele.Output["Body"]) {
					try {
                        ...
                        {
                            // 卡片类型 - 使用新的 cardFlags 字典
                            table.SetRaw(itemId, ServerItemAttributes.SubType, DbIOUtils.LoadFlag(item["SubType"], cardFlags, "0"));
                        }
                        else {
                            try
                            {
                            ...
                            {
                                // 添加详细的错误信息
                                string subTypeValue = item["SubType"]?.ObjectValue ?? "null";
                                string itemIdStr = itemId.ToString();
                                string itemName = item["Name"]?.ObjectValue ?? "未知";
                                string aegisName = item["AegisName"]?.ObjectValue ?? "未知";

                                string errorMsg = $"子类型解析失败 - 物品ID: {itemIdStr}, 名称: {itemName}, AegisName: {aegisName}, 子类型值: '{subTypeValue}'";

                                // 输出可用的子类型标志
                                string availableSubTypes = string.Join(", ", weaponFlags.Keys.Take(10));
                                errorMsg += $", 可用子类型: {availableSubTypes}...";

                                Console.WriteLine($"子类型解析错误: {errorMsg}");
                                // 移除了 Debug.WriteLine，避免命名空间冲突
        ...
        private static object _jobToIdYaml(AbstractDb<int> adb, ParserObject parser)
        {
            ...
            // 处理简单的布尔值 "true" 或 "false"
            if (value is ParserString)
            {
                string stringValue = value.ObjectValue;
                if (stringValue.ToLowerInvariant() == "true")
                {
                    return "0xFFFFFFFF"; // 所有职业
                }
            ...
            if (value is ParserArrayBase)
            {
                foreach (ParserKeyValue job in value.OfType<ParserKeyValue>())
                {
                    ...
                    {
                        throw new Exception("Unknown job : " + job.Key); // 修正：应该是 job.Key 而不是 job.ObjectValue
                    }
                }

                return "0x" + outputJob.ToString("X16");
            }
		...
		public static readonly Dictionary<string, long> ItemDbJobsYaml = new Dictionary<string, long> {
			{ "All", -1L },
			{ "Acolyte", 1L << 4 },
			{ "Alchemist", 1L << 18 },
			{ "Archer", 1L << 3 },
            ...
            { "Cardinal", 1L << 36 },
            { "Windhawk", 1L << 37 },
            { "Imperial_Guard", 1L << 38 },
            { "Biolo", 1L << 39 },
            { "Abyss_Chaser", 1L << 40 },
            { "Elemental_Master", 1L << 41 },
            { "Inquisitor", 1L << 42 },
            { "Troubadourtrouvere", 1L << 43 },
            { "Sky_Emperor", 1L << 44 },
            { "Soul_Ascetic", 1L << 45 },
            { "Shinkiro_Shiranui", 1L << 46 },
            { "Night_Watch", 1L << 47 },
            { "Hyper_Novice", 1L << 48 },
            { "Spirit_Handler", 1L << 49 },
        };
		...
		public static readonly Dictionary<string, int> ItemDbJobsYaml3 = new Dictionary<string, int> {
			...
			{ "Biolo", 1 << 39 },
			{ "Abyss_chaser", 1 << 40 },
			{ "Elemental_master", 1 << 41 },
			{ "Inquisitor", 1 << 42 },
			{ "Troubadourtrouvere", 1 << 43 },
			{ "Sky_emperor", 1 << 44 },
			{ "Soul_ascetic", 1 << 45 },
			{ "Shinkiro_shiranui", 1 << 46 },
			{ "Night_watch", 1 << 47 },
			{ "Hyper_novice", 1 << 48 },
			{ "Spirit_handler", 1 << 49 },
        };
